...
if __name__ == "__main__":
    ...
    def parse_cmdline():
        parser = argparse.ArgumentParser(add_help=True, formatter_class=argparse.RawTextHelpFormatter,
            description='A Python tool to match UUID in topology and FW, then find out whether all module in topoloy can be supported by this FW.\
 Currently it only supports IPC4 FW and IPC3 & IPC4 topology. The output includes FW version and with an error message if they are not matched.\
 And all the unsupported modules will be printed out.\
...
if __name__=="__main__":
    def main():
        ...
        for widget in tplg.widget_list:
            ...
            widget_uuid = tplg.get_widget_uuid(widget)
            found = False
            if widget_uuid is not None:
                for m in fw_content.modules:
                    if m.uuid == widget_uuid:
                        found = True
                        if cmd_args.verbose:
                ...
                if found == False:
                    not_found += 1
                    failed_uuid = uuid.UUID(bytes=widget_uuid)
                    print("Error: widget:", name, failed_uuid, "is not supported by this FW")

        if not_found == 0:
            print("This FW can support with this topology")
        else:
            print("Error: this FW can't support with this topology")
